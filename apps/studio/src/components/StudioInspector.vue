<template>
  <aside class="inspector" :class="{ 'inspector--empty': !node }" aria-label="领域对象检查器">
    <template v-if="node">
      <header class="inspector__header"><div><span class="eyebrow">INSPECTOR · {{ node.type }}</span><h2>{{ node.label }}</h2></div><button class="icon-button" type="button" aria-label="关闭检查器" @click="store.selectedNodeId = undefined"><X :size="17" /></button></header>
      ...
      <section v-if="episode" class="inspector__section"><h3>Episode 上下文</h3><dl class="inspector__facts"><div><dt>顺序</dt><dd>{{ episode.ordinal + 1 }}</dd></div><div><dt>Series</dt><dd>{{ store.snapshot?.series?.name ?? 'Standalone' }}</dd></div><div><dt>Revision</dt><dd>r{{ episode.revision }}</dd></div></dl><form v-if="!episode.seriesId" class="inspector-inline-form" @submit.prevent="store.createSeriesAndAttach(seriesName)"><input v-model="seriesName" required maxlength="160" placeholder="新 Series 名称" /><button class="secondary-button" type="submit">创建并加入</button></form></section>
      ...
      <section v-if="!event && !shot && !candidate && !task && !series && !episode && !resolvedAsset" class="inspector__section"><h3>结构化数据</h3><pre>{{ prettyEntity }}</pre></section>
    </template>
...
import type { Candidate, Episode, GenerationTask, PromptRun, ResolvedAsset, Series, SharedAsset, Shot, StoryEvent } from '@aigc-director/contracts'
