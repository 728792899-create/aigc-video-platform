...
评审范围：Figma v2 结构目标/当前运行时、16 Workspace、零 Key Demo、恢复/诊断、安全边界、1440/1180/≤768
...
| P0 | Runtime 只有三张领域图，目标页面无法深链接 | 建立 16 Workspace Registry 与 `workspace` 查询参数，保留旧 `view` | 单元测试与 Browser 前进/后退通过 |
...
| P0 | 窄屏阶段条与任务托盘重叠 | ≤768px 只保留单层底部导航，主操作改为单列全宽 | Browser 767：无横向溢出，不再显示重复品牌导航 |
| P0 | `导出与设置` 主操作误开系统面板 | 将主操作显式路由到 Delivery 预检，并增加源码契约测试 | 路由和自动化契约通过；原生目录选择与人工导出留待发布候选包复测 |
...
| 角色 | 本轮检查 | 当前结果 | 仍需人工确认 |
| --- | --- | --- | --- |
| 产品 | MVP 范围、16 Workspace 职责、主任务、Planned/External Gate | 代码/规格一致；无新增 API 承诺 | 业务优先级与术语最终签字 |
| 设计 | 8 流程、按钮文案、44px、Obsidian Atelier、响应式 | Browser 三档与收起侧栏实测通过 | Figma 节点级像素对比需先修复 MCP 文件访问 |
