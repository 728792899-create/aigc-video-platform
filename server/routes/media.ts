...
import { asRecord, errorMessage } from './routeSupport'
  ...
  try {
    ...
    const files = fs.readdirSync(dir)
      ...
      .map((file) => {
        ...
        return {
          key: file,
          url: `/uploads/bgm/${file}`,
          name: file.replace(/^bgm_[0-9a-f-]+/i, '').replace(/\.[^.]+$/, '') || file,
          size: st.size,
    ...
    res.status(500).json({ code: 500, data: null, message: `读取 BGM 失败: ${errorMessage(e)}` });
  }
});
...
router.post('/bgm', (req, res) => {
  upload.single('file')(req, res, (err: unknown) => {
    if (err) return res.status(400).json({ code: 400, data: null, message: errorMessage(err) });
...
router.get('/motions', (req, res) => {
  ...
  const data = Object.entries(presets).map(([key, value]) => ({
    key, label: asRecord(value).label, desc: asRecord(value).desc,
  }));
