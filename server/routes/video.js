...
function clampVideoSpeed(value) {
 ...
 */
function buildEncodeArgs(options = {}, extra = {}) {
 ...
 */
async function transcodeToFormat(inputPath, outputPath, options = {}) {
...
function normalizeMotion(motion) {
  ...
  const alias = { in: 'zoom-in', out: 'zoom-out', zoomin: 'zoom-in', zoomout: 'zoom-out',
 ...
 */
function buildMotionFilter(motion, w, h, fps, duration) {
 ...
 */
async function videoToSegment(videoPath, audioPath, outputPath, options = {}) {
  const fps = options.fps || 30;
  const { w, h } = options.resolution || resolveResolution(options.ratio);
...
/**
 * 把本地图片读成 data URL（base64），供图生视频（i2v）作首帧引导。
 * CogVideoX 的 image_url 接受公网 URL 或 data URL；本地分镜图无公网地址，故走 base64。
 * 超过 ~9MB 的图返回 null（base64 膨胀 ~33%，过大易触发云端 413），让上层退回纯文生视频。
 */
function imageToDataUrl(imagePath) {
  try {
    if (!imagePath || !fs.existsSync(imagePath)) return null;
    const stat = fs.statSync(imagePath);
    if (stat.size > 9 * 1024 * 1024) return null;
 ...
 */
function buildXfadeFilterComplex(segments, durations, transitions, transitionDuration) {
  ...
  for (let i = 1; i < segments.length; i++) {
    const transition = TRANSITION_MAP[transitions[i - 1]] || 'fade';
...
async function concatWithTransitions(segments, transitions, outputPath, options = {}) {
 ...
 * 给视频烧入字幕
 */
async function burnSubtitles(inputPath, outputPath, srtPath, style) {
 ...
 */
async function burnAss(inputPath, outputPath, assPath) {
  const assFilter = subtitleService.buildAssFilter(assPath);
  await ffmpeg('-y', '-i', inputPath, '-vf', assFilter,
               '-c:v', 'libx264', '-c:a', 'copy', '-pix_fmt', 'yuv420p', outputPath);
}

async function applyVideoSpeed(inputPath, outputPath, speed) {
 ...
 */
async function mixBgm(inputPath, bgmPath, outputPath, bgmVolume = 0.25) {
...
router.post('/compose', async (req, res) => {
  try {
    const { project_id, options = {}, async: asyncMode } = req.body;
    if (project_id == null || (typeof project_id !== 'number' && !/^\d+$/.test(String(project_id)))) {
    ...
    try {
      ...
      return res.status(400).json({ code: 400, data: { export_location: exportLocationInfo() }, message: `导出目录不可用: ${e.message}` });
    }
    ...
    const valid = storyboards.filter(sb => sb.image_path && fs.existsSync(sb.image_path));
    if (valid.length === 0) {
    ...
    // 异步模式：立即返回 task_id
    if (asyncMode) {
      let taskMeta = { project_id, total_segments: valid.length };
      try {
      ...
      doVideoCompose(task.id, project_id, valid, options).catch((e) => {
    ...
    res.status(500).json({ code: 500, data: null, message: `合成失败: ${err.message}` });
  }
});
...
router.post('/preview-compose', async (req, res) => {
  try {
    const { project_id, options = {}, limit = 3 } = req.body;
    if (project_id == null || (typeof project_id !== 'number' && !/^\d+$/.test(String(project_id)))) {
      return res.status(400).json({ code: 400, data: null, message: '缺少或非法的 project_id' });
    }
    const project = getDb().prepare('SELECT * FROM projects WHERE id=?').get(project_id);
    ...
    const allValid = storyboards.filter(sb => sb.image_path && fs.existsSync(sb.image_path));
    ...
    res.status(500).json({ code: 500, data: null, message: `快速预览失败: ${err.message}` });
  }
});

// 后台异步合成
async function doVideoCompose(taskId, project_id, valid, options) {
  ...
  try {
    ...
    const result = await composeVideo(project_id, valid, options, (progress, message) => {
 ...
 */
function resolveT2vTarget(options, isPreview) {
  ...
  if (videoSel && videoSel !== 'static') {
    ...
    const [prov, mdl] = raw.includes('__') ? raw.split('__') : [raw, videoSelModel];
 ...
 */
async function generateSegments(projectId, valid, t2vTarget, options, resolution, tempDir, timestamp, onProgress) {
  const segments = [];
  const transitions = [];
    ...
    if (shouldUseAiVideo) {
      ...
      try {
        ...
        if (options.i2v !== false && sb.image_path && fs.existsSync(sb.image_path)) {
          imageUrl = imageToDataUrl(sb.image_path) || undefined;
        }
        const gen = await t2v.generate({
        ...
        if (gen?.local_path && fs.existsSync(gen.local_path)) {
          ...
          try {
            ...
            try { assetNaming.normalizeStoryboardVideo(sb.id); } catch (e) { console.warn('[assetNaming] 分镜视频命名整理失败:', e.message); }
          } catch (e) {
            console.error(`[compose] 分镜 ${sb.id} 视频持久化失败:`, e.message);
            try { fs.unlinkSync(gen.local_path); } catch {}
          }
          segDone = true;
        }
      } catch (e) {
        console.error(`[compose] 分镜 ${sb.id} AI 视频生成失败，降级静图:`, e.message);
      }
    }

    if (!segDone) {
      // —— 静图运镜轨（默认 / T2V 降级兜底）——
      await withFfmpegTimeout(timeoutForSeconds(targetDuration, 'segment'), () => imageAudioToSegment(sb.image_path, audioPath, targetDuration, segPath, {
        fps: options.fps || 30,
        resolution,
...
function hasSubtitleText(storyboards = []) {
  return storyboards.some(sb => String(sb.subtitle_text || sb.dialog || '').trim());
}

function subtitleStyleFrom(valid, options = {}) {
...
function moveVideoOutput(inputPath, outputPath) {
  if (path.resolve(inputPath) === path.resolve(outputPath)) return;
  if (fs.existsSync(outputPath)) try { fs.unlinkSync(outputPath); } catch {}
  fs.renameSync(inputPath, outputPath);
}

function subtitleMetadata({ hasSubtitle, burnSubtitle, srtUrl = null, vttUrl = null, status = null, error = null } = {}) {
  return {
...
function generateSubtitleFiles(valid, projectId, timeline) {
...
async function burnSubtitleTrack(valid, projectId, inputPath, outputPath, options, timeline) {
 ...
 */
async function applySubtitles(valid, projectId, stageVideo, finalOutput, options, onProgress, timeline) {
  ...
  try {
    const files = burnSubtitle
      ? null
      : generateSubtitleFiles(valid, projectId, timeline);
    if (burnSubtitle) {
    ...
    if (burnSubtitle) {
      const err = new Error(`字幕烧录失败: ${e.message}`);
      err.subtitleMetadata = meta;
      throw err;
    }
    console.error('[compose] 软字幕生成失败，输出无外挂字幕版本:', e.message);
    moveVideoOutput(stageVideo, finalOutput);
    return meta;
  }
}
 ...
 */
async function applyBgm(videoIn, bgmPath, projectId, tempDir, timestamp, options, onProgress) {
  ...
  try {
    ...
    console.error('[compose] BGM 混音失败，跳过:', e.message);
    if (fs.existsSync(bgmOut)) try { fs.unlinkSync(bgmOut); } catch {}
    return videoIn;
  }
}

function timelineDurationForStoryboard(timeline, storyboardId, fallback = 5) {
...
function buildLongVideoGroups(valid, timeline, targetSec = 300) {
...
function buildChapterTimeline(timeline, storyboards = []) {
...
async function concatCopyVideos(files, outputPath, totalDurationSec) {
...
async function composeLongVideo(project_id, valid, options = {}, onProgress = () => {}) {
  ...
  const finalOutput = path.join(outputDir, `project_${project_id}_${timestamp}.mp4`);
  const tempVideo = path.join(tempDir, `long_${project_id}_${timestamp}.mp4`);
  const speedVideo = path.join(tempDir, `long_speed_${project_id}_${timestamp}.mp4`);
  ...
  try {
    for (let i = 0; i < groups.length; i++) {
      ...
      const result = await generateSegments(project_id, group.storyboards, t2vTarget, longOptions, resolution, tempDir, chapterStamp, (p, msg) => {
        const local = 8 + (52 * (i + (p || 0) / 100)) / groups.length;
        onProgress(Math.min(64, Math.round(local)), msg);
      });
      const chapterPath = path.join(tempDir, `chapter_${project_id}_${i}_${timestamp}.mp4`);
      ...
      if (longOptions.burnSubtitle !== false && hasSubtitleText(group.storyboards)) {
        chapterOutputPath = path.join(tempDir, `chapter_sub_${project_id}_${i}_${timestamp}.mp4`);
      ...
      try {
        const chapterRow = getDb().prepare('SELECT id FROM chapters WHERE project_id = ? AND chapter_index = ?').get(project_id, group.chapterIndex);
        if (chapterRow) {
    ...
    const bgmStageVideo = await applyBgm(tempVideo, bgmPath, project_id, tempDir, timestamp, longOptions, onProgress);
    ...
    if (longOptions.burnSubtitle !== false && hasSubtitle) {
      ...
      const files = generateSubtitleFiles(valid, project_id, outputTimeline);
      moveVideoOutput(stageVideo, finalOutput);
      ...
      subtitleMeta = await applySubtitles(valid, project_id, stageVideo, finalOutput, longOptions, onProgress, outputTimeline);
    }
    ...
    if (targetFmt.ext !== '.mp4') {
      try {
        ...
        console.error(`[compose-long] 格式转换失败，退回 mp4:`, e.message);
      }
    }
    ...
    const exportRes = getDb().prepare(
      ...
      project_id, relPath, relPath, 'success', outputTimeline.total_duration, groups.length, 1,
    ...
    const normalizedExportUrl = (() => {
      try { return assetNaming.normalizeExport(exportRes.lastInsertRowid); } catch (e) {
        console.warn('[assetNaming] 长视频命名整理失败:', e.message);
        return relPath;
      }
    })();
...
async function composeVideo(project_id, valid, options = {}, onProgress = () => {}) {
  ...
  if (shouldUseLongPipeline) {
    return composeLongVideo(project_id, valid, options, onProgress);
  }
  ...
  try {
    ...
    const result = await generateSegments(
      project_id, valid, t2vTarget, options, resolution, tempDir, timestamp, onProgress
    );
    ...
    const bgmStageVideo = await applyBgm(tempVideo, bgmPath, project_id, tempDir, timestamp, options, onProgress);
    ...
    const subtitleMeta = await applySubtitles(valid, project_id, stageVideo, finalOutput, options, onProgress, outputTimeline);
    ...
    if (!isPreview && targetFmt.ext !== '.mp4') {
      try {
        ...
        console.error(`[compose] 格式转换为 ${targetFmt.ext} 失败，退回 mp4:`, e.message);
      }
    }
    ...
    const exportRes = getDb().prepare(
      ...
      project_id, relPath, relPath, 'success', outputTimeline.total_duration,
      subtitleMeta.has_subtitle, subtitleMeta.burn_subtitle, subtitleMeta.srt_url, subtitleMeta.vtt_url,
      subtitleMeta.subtitle_status, subtitleMeta.subtitle_error, videoSpeed
    );
    const normalizedExportUrl = (() => {
      try { return assetNaming.normalizeExport(exportRes.lastInsertRowid); } catch (e) {
        console.warn('[assetNaming] 成片命名整理失败:', e.message);
        return relPath;
      }
    })();
    const finalUrl = normalizedExportUrl || relPath;
 ...
 */
async function composeProjectVideo(project_id, options = {}, onProgress = () => {}) {
  const storyboards = getDb().prepare(
    `SELECT s.*, i.file_path as image_path
     FROM storyboards s
     LEFT JOIN images i ON s.selected_image_id = i.id
  ...
  await acquireComposeLock();
  try {
    return await composeVideo(project_id, valid, options, onProgress);
  } finally {
    releaseComposeLock();
  }
}

function normalizeExportSubtitleState(row) {
  const r = { ...row };
...
router.get('/library', (req, res) => {
  try {
    ...
    res.status(500).json({ code: 500, data: null, message: `读取成片库失败: ${err.message}` });
  }
});
...
router.composeProjectVideo = composeProjectVideo;
router.MOTION_PRESETS = MOTION_PRESETS;
router.RATIO_PRESETS = RATIO_PRESETS;
