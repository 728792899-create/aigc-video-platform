...
function resolveCoverUrl(projectValue: unknown): string | null {
  const project = asRecord(projectValue);
  if (!project) return null;
  if (typeof project.cover === 'string' && project.cover) return project.cover;
  try {
    ...
    if (row && row.file_url) return sqlText(row.file_url) || null;
  } catch (_) { /* 表未就绪或无图，忽略 */ }
  return null;
}

function enrichProject(projectValue: unknown, includeHealth = true): JsonRecord | null {
  if (!projectValue) return null;
  const project = asRecord(projectValue);
  const enriched = asRecord(attachTimeMs(project));
  ...
  if (includeHealth) {
    ...
    if (enriched.asset_health && project.status !== 'generating') {
      const health = asRecord(enriched.asset_health);
      const counts = asRecord(health.counts);
      if ((Number(counts.exports) || 0) > 0) enriched.status = 'completed';
      else if ((Number(counts.storyboards) || 0) === 0) enriched.status = 'draft';
      else if (health.can_compose) enriched.status = 'ready';
      else if (health.status === 'error') enriched.status = 'partial';
...
router.get('/', (req, res) => {
  ...
  res.json({ code: 200, data: projects.map((project) => enrichProject(project, true)), message: 'success' });
});
...
router.get('/:id/workbench-status', (req, res) => {
  try {
    ...
    res.status(500).json({ code: 500, data: null, message: `读取工作台状态失败: ${errorMessage(err)}` });
  }
});
...
router.get('/:id/artifacts', (req, res) => {
  try {
    ...
    const rows = listStageArtifacts(req.params.id || '');
    // 默认只返回版本元数据，避免历史脚本/Prompt 随列表接口被重复传输。
    const exposePayload = req.query.include_payload === 'true';
    const present = (row: StageArtifactRow) => exposePayload ? row : Object.fromEntries(
      Object.entries(row).filter(([key]) => key !== 'payload')
    );
    res.json({
      code: 200,
      data: {
        current: rows.filter((row) => row.status === 'current').map(present),
        stale: rows.filter((row) => row.status === 'stale').map(present),
...
router.post('/:id/workbench/repair', async (req, res) => {
  try {
    ...
    const code = details.code === 'NO_REFERENCE_IMAGE' || details.code === 'NO_CHARACTER' ? 400 : 500;
    res.status(code).json({
      code,
      data: { advice: Array.isArray(details.advice) ? details.advice : [] },
      message: errorMessage(err) || '修复失败',
    });
  }
});

// 项目级批量生图：缺图补齐 / 全部重生 / 失败重试 / 低分重生。
router.post('/:id/images/generate-all', idempotency(), (req, res) => {
  try {
    ...
    const task = taskManager.create('image-batch', {
      ...
      mode: selected.mode,
      payload: req.body || {},
      target_storyboard_ids: selected.storyboards.map((storyboard: JsonRecord) => Number(storyboard.id)),
      ...
      demo_mode: demoMode,
      recovery: { kind: 'image-batch', mode: demoMode ? 'safe-auto' : 'manual-reconcile', attempts: 0, max_attempts: 3 },
    });
    ...
    res.status(500).json({ code: 500, data: null, message: `批量生图启动失败: ${errorMessage(err)}` });
  }
});
...
router.post('/:id/complete-check', (req, res) => {
  try {
    ...
    res.status(500).json({ code: 500, data: null, message: `项目状态检查失败: ${errorMessage(err)}` });
  }
});
...
router.get('/:id/story-bible', (req, res) => {
  try {
    ...
    res.status(500).json({ code: 500, data: null, message: `读取故事圣经失败: ${errorMessage(err)}` });
  }
});
...
router.put('/:id/story-bible', (req, res) => {
  try {
    ...
    res.status(500).json({ code: 500, data: null, message: `保存故事圣经失败: ${errorMessage(err)}` });
  }
});
...
router.get('/:id/characters', (req, res) => {
  try {
    ...
    res.status(500).json({ code: 500, data: null, message: `读取角色库失败: ${errorMessage(err)}` });
  }
});
...
router.post('/:id/characters/extract', (req, res) => {
  try {
    ...
    res.status(500).json({ code: 500, data: null, message: `提取角色失败: ${errorMessage(err)}` });
  }
});
...
router.post('/:id/characters/auto-lock', (req, res) => {
  try {
    ...
    res.status(code).json({
      code,
      data: { advice: Array.isArray(details.advice) ? details.advice : [] },
      message: errorMessage(err) || '一键定妆失败',
    });
  }
});
...
router.post('/:id/continue', (req, res) => {
  try {
    ...
    res.status(500).json({ code: 500, data: null, message: `创建续写项目失败: ${errorMessage(err)}` });
  }
});
...
router.post('/:id/continuity/check', (req, res) => {
  try {
    ...
    res.status(500).json({ code: 500, data: null, message: `一致性检查失败: ${errorMessage(err)}` });
  }
});
...
router.post('/:id/continuity/repair', async (req, res) => {
  try {
    ...
    res.status(500).json({ code: 500, data: null, message: `一致性修复失败: ${errorMessage(err)}` });
  }
});
...
router.get('/:id/series', (req, res) => {
  try {
    ...
    res.status(500).json({ code: 500, data: null, message: `读取系列视图失败: ${errorMessage(err)}` });
  }
});
...
router.get('/:id/timeline', async (req, res) => {
  try {
    ...
    res.status(500).json({ code: 500, data: null, message: `读取时间轴失败: ${errorMessage(err)}` });
  }
});
...
router.post('/:id/timeline/rebuild', async (req, res) => {
  try {
    ...
    res.status(500).json({ code: 500, data: null, message: `重建时间轴失败: ${errorMessage(err)}` });
  }
});
...
router.delete('/:id', (req, res) => {
  ...
  if (String(req.query.permanent) === 'true') {
    ...
    safeUnlinkMany(images.map((image) => image.file_path || image.file_url));
    safeUnlinkMany(audios.map((audio) => audio.audio_url));
    safeUnlinkMany(videos.map((video) => video.video_path));
    safeUnlinkMany(exportsRows.map((item) => item.file_path || item.file_url));
...
router.post('/:id/cover', async (req, res) => {
  ...
  try {
    ...
    if (typeof oldCover === 'string' && oldCover.startsWith('/uploads/') && oldCover !== fileUrl) {
    ...
    console.error('[project.cover] 生成失败:', errorMessage(err));
    res.status(502).json({ code: 502, data: null, message: `封面生成失败：${errorMessage(err)}` });
  }
});
