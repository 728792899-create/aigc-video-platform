...
function buildOwnerMap(db, type) {
  const map = {};
  const projRows = db.prepare('SELECT id, name FROM projects').all();
  const projName = {};
  for (const p of projRows) projName[p.id] = p.name;
  ...
  try {
    if (type === 'image') {
      ...
      for (const r of rows) if (r.url) map[r.url] = { pid: r.pid, name: projName[r.pid] || null };
    } else if (type === 'video') {
      ...
      for (const r of storyboardRows) if (r.url) map[r.url] = { pid: r.pid, name: projName[r.pid] || null };
    } else if (type === 'audio') {
    ...
    console.error('[files] 构建归属映射失败:', e.message);
  }
  return { map, projName };
}

// 文件名兜底解析项目号：project_N / tts_N_ / subtitle_project_N
function guessProjectFromName(name) {
...
router.get('/', (req, res) => {
  try {
    ...
    if (!TYPE_DIR[type]) return res.status(400).json({ code: 400, message: '无效的文件类型' });
...
router.post('/normalize-names', (req, res) => {
  try {
    ...
    res.status(500).json({ code: 500, data: null, message: `整理素材命名失败: ${e.message}` });
  }
});
...
router.delete('/', (req, res) => {
  try {
    ...
    if (permanent) {
      ...
      try {
        const cleanupDb = db.transaction((urls) => {
        ...
        console.error('[files] 清理 DB 引用失败（已回滚）:', e.message);
      }
      return res.json({
        code: 200,
...
router.get('/scripts', (req, res) => {
  try {
    ...
    const list = projects.map((p) => {
      ...
      let charCount = (p.script_content || '').length;
      for (const s of sbs) charCount += (s.description || '').length + (s.dialog || '').length;
      return {
        project_id: p.id,
        name: p.name,
        theme: p.theme,
        ...
        has_script: !!(p.script_content || sbs.length),
...
// 取单项目剧本详情（含全部分镜）
function loadScript(db, projectId) {
  const p = db.prepare(
...
router.get('/scripts/:projectId/export', (req, res) => {
  try {
    ...
    for (const s of data.storyboards) {
      lines.push('', `【场景 ${s.scene_number}】（${s.duration || 0}s）`);
      if (s.description) lines.push(`画面：${s.description}`);
      if (s.dialog) lines.push(`对白：${s.dialog}`);
