...
import { errorMessage, parseJsonRecord, sqlText } from './routeSupport'
...
function ensureBuiltins() {
  ...
  const existing = new Set(
    db.prepare("SELECT name FROM presets WHERE is_builtin=1").all().map((r) => sqlText(r.name))
  );
  ...
  const run = db.transaction((list: typeof BUILTIN_PRESETS) => {
...
function parseRow(r: SqlRow) {
  const cfg = parseJsonRecord(r.config);
...
router.get('/', (req, res) => {
  try {
    ...
    res.status(500).json({ code: 500, data: null, message: `读取预设失败: ${errorMessage(err)}` });
  }
});
...
router.post('/', (req, res) => {
  try {
    ...
    if (!row) throw new Error('预设保存后无法读取');
    res.json({ code: 200, data: parseRow(row), message: '预设已保存' });
  } catch (err) {
    res.status(500).json({ code: 500, data: null, message: `保存预设失败: ${errorMessage(err)}` });
  }
});
...
router.put('/:id', (req, res) => {
  try {
    ...
    if (!updated) throw new Error('预设更新后无法读取');
    res.json({ code: 200, data: parseRow(updated), message: '已更新' });
  } catch (err) {
    res.status(500).json({ code: 500, data: null, message: `更新预设失败: ${errorMessage(err)}` });
  }
});
...
router.delete('/:id', (req, res) => {
  try {
    ...
    res.status(500).json({ code: 500, data: null, message: `删除预设失败: ${errorMessage(err)}` });
  }
});
