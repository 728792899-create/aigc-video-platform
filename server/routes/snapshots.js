...
router.get('/project/:projectId', (req, res) => {
  try {
    ...
    const countMap = {};
    for (const f of full) {
      try { countMap[f.id] = (JSON.parse(f.snapshot).storyboards || []).length; } catch { countMap[f.id] = 0; }
    }
    res.json({ code: 200, data: rows.map(r => ({ ...r, storyboard_count: countMap[r.id] || 0 })), message: 'success' });
  } catch (err) {
    res.status(500).json({ code: 500, data: null, message: `读取快照失败: ${err.message}` });
  }
});
...
router.post('/project/:projectId', (req, res) => {
  try {
    ...
    const sbWithImages = storyboards.map(sb => ({ ...sb, _images: imgStmt.all(sb.id) }));
    ...
    res.status(500).json({ code: 500, data: null, message: `创建快照失败: ${err.message}` });
  }
});
...
router.post('/:id/restore', (req, res) => {
  try {
    ...
    let data;
    try { data = JSON.parse(snap.snapshot); } catch { return res.status(500).json({ code: 500, data: null, message: '快照数据损坏' }); }
    const projectId = snap.project_id;
    const sbs = data.storyboards || [];
    const db = getDb();
    ...
    try { safeUnlinkMany(oldVideos.map(v => v.video_path)); } catch (_) { /* 清理失败不阻断回滚结果 */ }
    res.json({ code: 200, data: { project_id: projectId, restored: sbs.length }, message: '已回滚到该快照' });
  } catch (err) {
    res.status(500).json({ code: 500, data: null, message: `回滚失败: ${err.message}` });
  }
});
...
router.delete('/:id', (req, res) => {
  try {
    ...
    res.status(500).json({ code: 500, data: null, message: `删除快照失败: ${err.message}` });
  }
});
