...
function cleanupFailedAutoProduce(projectId) {
  try {
    ...
    console.error('[auto-produce] 清理失败项目出错:', e.message);
  }
...
function autoProduceDefaults(body) {
...
function finishAutoProduceFailure(taskId, projectId, err) {
  const task = taskManager.get(taskId);
  const diagnosis = failureAdvisor.diagnose(err, {
    projectId,
    taskId,
    currentMessage: task && task.message,
    stageHint: err && err.stageHint,
    assetHealth: err && err.assetHealth,
  });
  const cleanup = cleanupFailedAutoProduce(projectId);
  const error = err instanceof Error ? err : new Error(String(err || '一键成片失败'));
...
async function runAutoProduceTask(taskId, projectId, produceParams) {
...
// AI文案生成
router.post('/generate-script', validateBody(schemas.generateScript), async (req, res) => {
  try {
...
router.post('/expand-dialog', async (req, res) => {
  try {
...
router.post('/optimize-theme', validateBody(schemas.optimizeTheme), async (req, res) => {
  try {
...
router.post('/generate-image', idempotency({ ttlMs: 5 * 60 * 1000 }), validateBody(schemas.generateImage), async (req, res) => {
  try {
    const {
    ...
    if (asyncMode) {
      ...
      doImageGeneration(task.id, sb, style, effectivePrompt, ratio, model, storyboard_id, batch_size, '', imageSeed, continuityContext, {
        compiledPrompt,
        autoSelectBest: shouldAutoSelectBest,
    ...
    if (shouldReuseCache) {
      ...
      const cachedPayload = applyCachedImageResult(storyboard_id, cached, shouldAutoSelectBest);
      if (cachedPayload) {
        return res.json({
          code: 200,
          data: {
...
// 异步图片生成后台执行
async function doImageGeneration(taskId, sb, style, userPrompt, ratio, model, storyboard_id, batchSize, visualAnchor = '', seed = null, continuityContext = null, options = {}) {
  try {
    ...
    if (options.reuseCache) {
      ...
      const cachedPayload = applyCachedImageResult(storyboard_id, cached, options.autoSelectBest);
      if (cachedPayload) {
...
// 共用的落库逻辑
function saveImageResults(storyboard_id, result) {
  const insertedIds = [];
  for (const lf of result.local_files) {
    ...
    try {
      ...
      console.warn('[assetNaming] 图片命名整理失败:', e.message);
    }
  }
  getDb().prepare('UPDATE storyboards SET prompt = ? WHERE id = ?')
    .run(result.prompt, storyboard_id);
  // ③ 自动选图：若该分镜还没有选中图片，自动选中刚生成的第一张，
  // 避免用户忘记在图片页点「使用」导致导出时报「没有可用的图片」。
  if (insertedIds.length > 0) {
    const sb = getDb().prepare('SELECT selected_image_id FROM storyboards WHERE id = ?').get(storyboard_id);
    if (sb && !sb.selected_image_id) {
      getDb().prepare('UPDATE storyboards SET selected_image_id = ? WHERE id = ?')
        .run(insertedIds[0], storyboard_id);
    }
  }
  return insertedIds;
}
...
function autoSelectBestImage(storyboardId, insertedIds = [], checks = []) {
  ...
  const rows = insertedIds
    .map((id) => getDb().prepare('SELECT * FROM images WHERE id = ?').get(id))
    .filter(Boolean);
...
function applyCachedImageResult(storyboardId, cached, autoSelectBest = false) {
  const ids = (cached?.result?.image_ids || []).map(Number).filter(Boolean);
  if (!ids.length) return null;
  const rows = ids.map((id) => getDb().prepare('SELECT * FROM images WHERE id = ?').get(id)).filter(Boolean);
  if (!rows.length) return null;
  const selectedId = cached.result.selected_image_id || rows[0].id;
  const selected = rows.find((r) => Number(r.id) === Number(selectedId)) || rows[0];
  const current = getDb().prepare('SELECT selected_image_id FROM storyboards WHERE id = ?').get(storyboardId);
  if (autoSelectBest || !current?.selected_image_id) {
    getDb().prepare('UPDATE storyboards SET selected_image_id = ?, prompt = ? WHERE id = ?')
      .run(selected.id, cached.result.prompt || cached.prompt || '', storyboardId);
  }
  return {
    reused_cache: true,
    cache_id: cached.id,
    image_count: rows.length,
    image_ids: rows.map((r) => r.id),
    selected_image_id: selected.id,
    files: rows.map((r) => ({ file_url: r.file_url, file_path: r.file_path })),
...
router.post('/auto-produce', idempotency({ ttlMs: 5 * 60 * 1000 }), validateBody(schemas.autoProduce), async (req, res) => {
  try {
    ...
    try {
      ...
      console.warn('[auto-produce] 预检 API Key 异常（放行继续）:', e.message);
    }
...
router.post('/auto-produce/:taskId/retry', async (req, res) => {
  try {
  ...
  try {
    ...
    if (storyboard_id && result.file_url) {
      ...
      try {
        ...
        console.warn('[assetNaming] 配音命名整理失败:', e.message);
      }
    }
...
router.get('/voices', (req, res) => {
  ...
  const emotions = Object.entries(edgeTtsPro.EMOTION_PRESETS).map(([key, v]) => ({ key, label: v.label }));
  ...
  if (prov === 'volcano_tts_v3') {
    ...
    const keys = (def.models && def.models.length) ? def.models : Object.keys(VOLCANO_V3_LABELS);
    const voices = keys.map(k => ({ key: k, label: VOLCANO_V3_LABELS[k] || k, provider: 'volcano_tts_v3' }));
