...
function readVersion() {
  const resourcesPath = asRecord(process).resourcesPath;
  const packagedResources = typeof resourcesPath === 'string' ? resourcesPath : '';
  const candidates = [
    path.resolve(packagedResources, 'app', 'package.json'),
    path.resolve(packagedResources, 'app.asar', 'package.json'),
...
router.get('/diagnostics', (req, res) => {
  const limit = Math.min(parseInt(queryText(req.query.limit), 10) || 200, 1000);
