...
import * as edge from './tts' // 现有 Edge 引擎：generateTTS + VOICES
import type { TtsResult } from './tts'
const { resolveFfmpegPath } = require('../utils/ffmpeg');

type JsonObject = Record<string, unknown>
interface CloudTtsOptions { emotion?: string; speed?: number; volume?: number }
...
function generateSilentDemoTts(storyboardId: string | number | undefined, text = '', speed = 1): TtsResult {
...
function demoVoiceName(voice: string): string {
...
function generateMacosDemoTts(text: string, voice: string, speed: number, storyboardId?: string | number): TtsResult {
...
async function generateDemoTts(text: string, voice: string, speed: number, pitch: number, storyboardId: string | number | undefined, ttsOpts: CloudTtsOptions): Promise<TtsResult> {
  ...
  try {
    ...
    console.warn('[ttsProvider] demo TTS 使用本地静音占位:', errorMessage(localError));
...
async function genOpenAiTts(cred: ProviderCredentials, model: string, text: string, voice: string, storyboardId?: string | number): Promise<TtsResult> {
...
const VOLCANO_EMOTION: Record<string, string> = {
...
function mapVolcanoVoice(voice: string, model: string): string {
 ...
 */
async function genVolcanoTts(cred: ProviderCredentials, model: string, text: string, voice: string, storyboardId: string | number | undefined, { emotion = 'general', speed = 1.0, volume = 1.0 }: CloudTtsOptions = {}): Promise<TtsResult> {
  ...
  const json = asRecord(await resp.json().catch(() => ({})));
...
function mapVolcanoVoiceV3(voice: string, model: string): string {
 ...
 */
async function genVolcanoTtsV3(cred: ProviderCredentials, model: string, text: string, voice: string, storyboardId: string | number | undefined, { speed = 1.0 }: CloudTtsOptions = {}): Promise<TtsResult> {
  const base = (cred.baseUrl || 'https://openspeech.bytedance.com').replace(/\/+$/, '');
  ...
  for (let i = 0; i < txt.length; i++) {
    ...
    if (ch === '{') { if (depth === 0) start = i; depth++; }
      ...
      if (depth === 0 && start >= 0) {
        ...
        start = -1;
        try {
          const j = asRecord(JSON.parse(slice));
          if (typeof j.data === 'string') buffers.push(Buffer.from(j.data, 'base64'));
          ...
          if (header.code !== undefined && header.code !== 0 && header.code !== 20000000) {
            errMsg = `code=${header.code} ${header.message || ''}`;
          }
        } catch (_) { /* 跳过无法解析的片段 */ }
      }
    }
 ...
 */
export async function synthesize({ text = '', voice = 'xiaoxiao', speed = 1.0, pitch = 0, storyboardId, provider, model, noFallback = false, emotion = 'general', volume = 1.0 }: SynthesizeOptions = {}): Promise<TtsResult> {
  ...
  let useModel = model;
  if (!prov) {
    const route = asRecord(config.get('stageModels.voice'));
    prov = typeof route.provider === 'string' ? route.provider : 'edge';
    useModel = useModel || (typeof route.model === 'string' ? route.model : undefined);
  }
  ...
  try {
    if (def && def.protocol === 'openai-tts' && cred && cred.apiKey) {
      return await genOpenAiTts(cred, useModel || def.models[0] || 'tts-1', text, voice, storyboardId);
    }
    if (def && def.protocol === 'volcano-tts' && cred && cred.apiKey && cred.appId) {
      return await genVolcanoTts(cred, useModel || def.models[0] || 'BV700_streaming', text, voice, storyboardId, { emotion, speed, volume });
    }
    if (def && def.protocol === 'volcano-tts-v3' && cred && cred.apiKey && cred.appId) {
      return await genVolcanoTtsV3(cred, useModel || def.models[0] || 'zh_female_meilinvyou_moon_bigtts', text, voice, storyboardId, { speed });
    }
    // 未知协议或未配密钥：降级
    throw new Error(`配音 provider「${prov}」不可用，降级 Edge`);
  } catch (e) {
    if (noFallback) throw e;
    console.error(`[ttsProvider] 云端配音失败（${prov}），降级 Edge:`, errorMessage(e));
    return edge.generateTTS(text, voice, speed, pitch, storyboardId, ttsOpts);
  }
}

export const VOICES = edge.VOICES
