...
interface PodcastCredentials { appId: string; apiKey: string }
interface PodcastRound extends JsonObject {
  round_id?: unknown
  speaker?: unknown
  text?: unknown
  round_type?: unknown
...
type ParsedFrame =
  | { kind: 'error'; event: number | null; code: number; msg: string }
  | { kind: 'audio'; event: number | null; data: Buffer }
  | { kind: 'json'; event: number | null; data?: unknown; raw?: string }
  | { error: string }
 ...
 */
function buildConnectFrame(eventNum: number, payloadObj: JsonObject): Buffer {
 ...
 */
function buildSessionFrame(eventNum: number, sessionId: string, payloadObj: JsonObject): Buffer {
...
function parseFrame(buf: Buffer): ParsedFrame {
  ...
  const marker = buf[1] ?? 0;
  const msgType = marker >> 4;
  const flags = marker & 0x0f;
 ...
 */
export async function generatePodcast(options: PodcastOptions): Promise<PodcastResult> {
  const {
    credentials,
    action = 0,
    inputText = '',
  ...
  const rounds: PodcastRound[] = []; // [{round_id, speaker, text, audio_duration, start_time, end_time}, ...]
  let totalDuration = 0;
  ...
  return new Promise<PodcastResult>((resolve, reject) => {
    ...
    ws.on('message', (data: unknown) => {
      ...
      if ('kind' in frame && frame.kind === 'error') {
        clearTimeout(timer);
        ws.close();
      ...
      if ('kind' in frame && frame.kind === 'audio') {
        // event 361 PodcastAudio
        audioChunks.push(frame.data);
        return;
      }

      if ('kind' in frame && frame.kind === 'json') {
        const { event } = frame;
        ...
        if (event === 50) {
          ...
          if (lastRound) {
            ...
            totalDuration = Number(payload.end_time) || totalDuration;
          }
        } else if (event === 363) {
