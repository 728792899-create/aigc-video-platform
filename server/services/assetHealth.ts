...
export interface AssetIssue {
  ...
  code: string
  message: string
  scenes?: Array<{ id: unknown; scene_number: unknown }>
  suggestions?: string[]
}
export interface ProjectAssetHealth {
  ...
  ffmpeg: { ok: boolean; source: string; path: string }
  output: { dir: string; writable: boolean }
}
export class AssetHealthError extends Error {
  assetHealth: ProjectAssetHealth | null = null
  stageHint?: 'compose' | 'asset'
}

function fileState(stored: unknown) {
...
export function checkProjectAssets(projectId: unknown): ProjectAssetHealth | null {
  const db = getDb();
  ...
  const imagesByStoryboard = new Map<unknown, SqlRow[]>();
  for (const img of images) {
    const group = imagesByStoryboard.get(img.storyboard_id) || [];
    group.push(img);
    imagesByStoryboard.set(img.storyboard_id, group);
  }
  ...
  for (const sb of storyboards) {
    ...
    if (String(sb.subtitle_text || sb.dialog || '').trim()) subtitleCount++;
  ...
  const outputDir = path.resolve(String(config.get('uploadDir')), 'videos');
...
export function assertComposable(projectId: unknown): ProjectAssetHealth {
  const health = checkProjectAssets(projectId);
  if (!health) {
    const err = new AssetHealthError('项目不存在，无法进行资产预检');
    err.assetHealth = null;
    throw err;
  }
  if (!health.can_compose) {
    const primary = health.issues.find((i) => i.level === 'error') || health.issues[0];
    const err = new AssetHealthError(primary ? primary.message : '项目资产未通过预检，无法合成视频');
