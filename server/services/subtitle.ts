...
interface Storyboard {
  ...
  audio_words?: string | WordTiming[] | null
}
...
interface Timeline { total_duration?: number; scenes?: unknown[] }
...
interface SubtitleStyle extends SubtitleOptions {
  fontSize?: number
  fontFamily?: string
  bold?: boolean
  outline?: number
...
interface TimedSegment { text: string; start: number; end: number }
interface EffectContext { w: number; h: number; durMs: number; marginV: number }
type SubtitleEffect = (_segment: TimedSegment, context: EffectContext) => string

function parseWords(value: Storyboard['audio_words']): WordTiming[] {
  let source: unknown = value
  if (typeof source === 'string') {
    try { source = JSON.parse(source) } catch { return [] }
  }
  if (!Array.isArray(source)) return []
  return source.flatMap((item): WordTiming[] => {
    if (!item || typeof item !== 'object' || Array.isArray(item)) return []
    const raw = Object.fromEntries(Object.entries(item))
    const start = Number(raw.start)
    const end = Number(raw.end)
    if (!Number.isFinite(start) || !Number.isFinite(end)) return []
    return [{ part: String(raw.part || ''), start, end }]
  })
}

// 复用 TTS 的说话人标记剥离逻辑：字幕显示也不应出现 （旁白）/（小精灵）/【画外音】 等说话人提示
let stripSpeakerTags = (text: unknown): string => String(text || '');
try { stripSpeakerTags = require('./tts').stripSpeakerTags || stripSpeakerTags; } catch { /* tts 未就绪时降级为原文 */ }

const UPLOAD_DIR = path.resolve(String(config.get('uploadDir')), 'subtitles');
 ...
 */
export function splitIntoSegments(text: unknown, maxLen = 15): string[] {
  if (!text) return [];
 ...
 */
function buildSrtEntry(text: string, startTime: number, duration: number, index: number): string {
 ...
 */
export function segmentsWithTiming(sb: Storyboard, sceneStart: number, sceneDuration: number, maxLen = 15, timingScale = 1): TimedSegment[] {
  ...
  // 解析词级时间戳
  const words = parseWords(sb.audio_words);
  ...
  if (words && words.length) {
    ...
    for (const seg of segs) {
      ...
      while (wi < words.length && acc < segChars) {
        const word = words[wi];
        if (!word) break;
        acc += word.part.length;
        wi++;
      }
      ...
      if (first && last) {
        result.push({
          text: seg,
          start: sceneStart + (first.start / 1000) * scale,
          end: sceneStart + (last.end / 1000) * scale,
        });
      }
    }
 ...
 */
export function generateSrt(storyboards: Storyboard[], projectId: string | number, opts: SubtitleOptions = {}) {
  const maxLen = opts.maxLen || 15;
  let index = 1;
  const entries: string[] = [];
  const timelineMap: Map<number, TimelineScene> = timelineService.sceneMap(opts.timeline);
 ...
 */
export function generateVtt(storyboards: Storyboard[], projectId: string | number, opts: SubtitleOptions = {}) {
  const maxLen = opts.maxLen || 15;
...
export function buildForceStyle(style: SubtitleStyle = {}): string {
  ...
  const alignMap: Record<SubtitlePosition, number> = { bottom: 2, top: 8, middle: 5 };
  const alignment = style.position ? alignMap[style.position] : 2;
 ...
 * @param {object} style 字幕样式
 */
export function generateKaraokeAss(storyboards: Storyboard[], projectId: string | number, style: SubtitleStyle = {}) {
  ...
  const alignMap: Record<SubtitlePosition, number> = { bottom: 2, top: 8, middle: 5 };
  const align = style.position ? alignMap[style.position] : 2;
  ...
  const lines: string[] = [];
  const timelineMap: Map<number, TimelineScene> = timelineService.sceneMap(style.timeline);
  let cursorMs = 0;
  for (const sb of storyboards) {
    const scene = timelineMap.get(Number(sb.id));
    ...
    if (words && words.length) {
      ...
      let wi = 0;
      for (const seg of segs) {
        const segWords: WordTiming[] = [];
        while (wi < words.length) {
          const word = words[wi];
          if (!word || (currentTime + (word.start / 1000) * timingScale) >= seg.end - 0.001) break;
          segWords.push(word); wi++;
        }
 ...
 */
export function buildAssFilter(assPath: string): string {
 ...
 */
const SUB_EFFECTS: Record<string, SubtitleEffect> = {
  none: () => '',
  // 淡入淡出：最常用，入出各 350ms
  fade: () => `{\\fad(350,350)}`,
  // 上浮淡入 + 末尾淡出：从底边下方 40px 上浮到位
  floatup: (_segment, ctx) => {
...
export const SUB_EFFECT_KEYS = Object.keys(SUB_EFFECTS);
 ...
 */
export function generateEffectAss(storyboards: Storyboard[], projectId: string | number, effect = 'fade', style: SubtitleStyle = {}) {
  const fontName = style.fontFamily || 'Microsoft YaHei';
  const fontSize = Number(style.fontSize) || MOVIE_SUBTITLE.fontSize;
  const W = 1280, H = 720;
  const marginV = Number(style.marginV) || MOVIE_SUBTITLE.marginV;
  const outlineWidth = Number(style.outline ?? style.outlineWidth) || MOVIE_SUBTITLE.outline;
  const shadow = Number(style.shadow) || MOVIE_SUBTITLE.shadow;
  const toAss = (hex: string | undefined, fallback: string): string => { const c = (hex || fallback).replace('#', ''); return `&H00${c.slice(4,6)}${c.slice(2,4)}${c.slice(0,2)}`; };
