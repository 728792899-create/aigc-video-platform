...
const { validateStructuredScript } = require('./scriptContract');

type DetailLevel = 'concise' | 'standard' | 'rich'
...
interface Chapter extends JsonObject {
...
interface Storyboard extends JsonObject {
  scene_number?: number
  chapter_index?: number
  chapter?: number
...
interface NarrationStats {
  ...
  storyboard_duration_sec: number
  estimated_narration_sec: number
  target_duration_sec: number
  target_min_sec: number
  target_max_sec: number
  narration_coverage: number
  min_coverage: number
}

interface ScriptResult extends JsonObject {
  storyboards?: Storyboard[]
  chapters?: Chapter[]
  quality_warnings?: string[]
  _warnings?: string[]
  narration_stats?: NarrationStats
}

interface ScriptOverride extends JsonObject {
  provider?: string
  model?: string
...
function errorCode(value: unknown): string | undefined {
  return isJsonObject(value) && typeof value.code === 'string' ? value.code : undefined
}

function storyboardsFromUnknown(value: unknown): Storyboard[] {
  if (!Array.isArray(value)) return []
  return value.filter(isJsonObject).map((item) => ({ ...item }))
}

function scriptResultFromUnknown(value: unknown): ScriptResult | null {
  if (!isJsonObject(value)) return null
  const result: ScriptResult = { ...value }
  if (Array.isArray(value.storyboards)) result.storyboards = storyboardsFromUnknown(value.storyboards)
  return result
}
...
function demoScript(theme: unknown, duration: string | number = '45-60', style = '写实', detailLevel: DetailLevel = 'standard'): ScriptResult {
...
function parseDurationRange(duration: unknown): DurationInfo {
...
const LONG_NARRATION_CPS: Record<DetailLevel, number> = {
  concise: 3.5,
  standard: 4.25,
  rich: 5.1,
};

function narrationCps(detailLevel: DetailLevel = 'standard'): number {
...
function estimateNarrationSeconds(text: unknown, detailLevel: DetailLevel = 'standard'): number {
...
function longSceneSeconds(durationInfo: DurationInfo): number {
  const target = Number(durationInfo?.target || 600);
  if (target >= 600) return 30;
  return 24;
}

function longChapterCount(targetSec: number): number {
  return Math.max(2, Math.ceil(targetSec / 300));
}

function splitDuration(totalSec: number, count: number): number[] {
  const base = Math.floor(totalSec / count);
  const chunks = Array.from({ length: count }, () => base);
  let rest = totalSec - base * count;
  for (let i = 0; i < chunks.length && rest > 0; i++, rest--) chunks[i] = (chunks[i] ?? 0) + 1;
  return chunks;
}

function defaultLongChapters(theme: unknown, durationInfo: DurationInfo): Chapter[] {
  ...
  return chapterSeconds.map((target_duration_sec, index) => {
    const angle = (index === chapterSeconds.length - 1
      ...
      : chapterAngles[index % (chapterAngles.length - 1)]) ?? ['内容展开', '按信息层次自然推进'];
    return {
...
function longSceneBeats(index: number, total: number): SceneBeat {
  ...
  return beats[(index - 2) % beats.length] ?? beats[0] ?? { key: 'context', label: '背景解释', visual: '展示主题背景' };
}

function longSceneExpansionSentences(theme: unknown, chapter: Chapter, _beat: SceneBeat, localIndex: number): string[] {
...
function fitDialogToTarget(seed: string, targetChars: number, theme: unknown, chapter: Chapter, beat: SceneBeat, localIndex: number): string {
...
function fallbackLongDialog(theme: unknown, chapter: Chapter, beat: SceneBeat, localIndex: number, _sceneCount: number, targetSeconds: number, detailLevel: DetailLevel): string {
  const cleanTheme = String(theme || '这个主题').trim() || '这个主题';
...
function fallbackLongScene(theme: unknown, chapter: Chapter, localIndex: number, sceneCount: number, targetSeconds: number, style: string, detailLevel: DetailLevel): Storyboard {
...
function fallbackLongChapterStoryboards(theme: unknown, chapter: Chapter, style: string, detailLevel: DetailLevel): Storyboard[] {
...
function longScriptScaffold(theme: unknown, duration: string | number = '3600-3600', style = '写实', detailLevel: DetailLevel = 'standard', extraWarnings: string[] = []): ScriptResult {
  const cleanTheme = String(theme || '长视频主题').trim() || '长视频主题';
  const durationInfo = parseDurationRange(duration);
...
function resolveScriptModel(override: ScriptOverride | null | undefined): { provider: string; model?: string } {
...
const DETAIL_LEVELS: Record<DetailLevel, { label: string; density: [number, number]; words: string }> = {
...
function estimateSceneSeconds(durationInfo: DurationInfo): number {
...
function dialogWordRange(detailLevel: DetailLevel, durationInfo: DurationInfo): [number, number] {
...
function buildTimingPlan(durationInfo: DurationInfo | null | undefined, detailLevel: DetailLevel = 'standard') {
...
function detailInstruction(detailLevel: DetailLevel, durationInfo: DurationInfo): string {
  const detail = DETAIL_LEVELS[detailLevel] || DETAIL_LEVELS.standard;
  const timing = buildTimingPlan(durationInfo, detailLevel);
...
function totalStoryboardDuration(result: ScriptResult): number {
  return (result?.storyboards || []).reduce((sum, item) => sum + (Number(item.duration) || 0), 0);
}

function buildChaptersFromStoryboards(storyboards: Storyboard[] = []): Chapter[] {
...
function hasChapterStructure(result: ScriptResult): boolean {
...
function dialogSignature(text: unknown): string {
  return String(text || '')
    .replace(/[「」『』“”"'，。！？；：,.!?;:\s]/g, '')
    .slice(0, 36);
}

function hasConsecutiveRepeatedDialog(storyboards: Storyboard[] = []): boolean {
...
function narrationStatsForStoryboards(storyboards: Storyboard[] = [], durationInfo: DurationInfo, detailLevel: DetailLevel = 'standard'): NarrationStats {
...
function normalizeLongStoryboards(storyboards: Storyboard[] = [], detailLevel: DetailLevel = 'standard'): Storyboard[] {
...
function enrichLongScriptResult(result: ScriptResult, durationInfo: DurationInfo, detailLevel: DetailLevel = 'standard', extraWarnings: string[] = []): ScriptResult & { narration_stats: NarrationStats; storyboards: Storyboard[] } {
...
function validateLongScriptResult(result: ScriptResult | null, theme: unknown, duration: string | number, style: string, detailLevel: DetailLevel, durationInfo: DurationInfo): ScriptResult | null {
...
function parseJsonObject(content: unknown): JsonObject | null {
  try {
...
async function generateLongOutline(
  theme: unknown,
  durationInfo: DurationInfo,
  style: string,
  _override: ScriptOverride,
  provider: string,
  model?: string,
): Promise<Chapter[]> {
...
3. 返回严格 JSON：{"chapters":[{"title":"第 1 章：...","summary":"..."}]}`;
  try {
    ...
    return fallback.map((chapter, index) => {
      const candidate = isJsonObject(incoming[index]) ? incoming[index] : {};
      return {
        ...chapter,
        title: typeof candidate.title === 'string' && candidate.title.trim() ? candidate.title : chapter.title,
        summary: typeof candidate.summary === 'string' && candidate.summary.trim() ? candidate.summary : chapter.summary,
      };
    });
  } catch {
    return fallback;
...
5. 返回严格 JSON：{"storyboards":[{"description":"","dialog":"","duration":24,"chapter_index":${chapter.chapter_index},"chapter_title":"${chapter.title}","characters_in_scene":[],"continuity_notes":"","scene_state_before":"","scene_state_after":""}]}${skillPrompt}${continuityContext}`;
  ...
  try {
    ...
    const parsed = parseJsonObject(content);
    const storyboards = storyboardsFromUnknown(parsed?.storyboards);
...
function buildSystemPrompt(style: string, detailLevel: DetailLevel = 'standard', skillPrompt = '', continuityContext = '', durationInfo: DurationInfo | null = null): string {
  const resolvedDuration = durationInfo || parseDurationRange('150-210');
  const timing = buildTimingPlan(resolvedDuration, detailLevel);
  const detailText = detailInstruction(detailLevel, resolvedDuration);
 ...
 */
async function generateScript(theme: unknown, duration: string | number = '150-210', style = '写实', override: ScriptOverride | null = null): Promise<JsonObject> {
  const durationInfo = parseDurationRange(duration);
  const detailLevel = (override && override.detailLevel) || 'standard';
  const demo = isDemoMode();
  const contractContext = (provider: string, model?: string) => ({
    theme,
    duration,
    style,
    detailLevel,
    language: (override && override.language) || 'zh-CN',
    provider,
    model,
  });
  if (durationInfo.max >= 600) {
    const result = await generateLongScript(theme, duration, style, override || {}, durationInfo, detailLevel);
    if (!result) throw new Error('长视频脚本生成结果为空');
    const resolved = demo ? { provider: 'demo', model: 'local-template' } : resolveScriptModel(override);
    return validateStructuredScript(result, contractContext(resolved.provider, resolved.model));
  }
  if (demo) {
    return validateStructuredScript(
      demoScript(theme, duration, style, detailLevel),
      contractContext('demo', 'local-template')
    );
  }
  const { provider, model } = resolveScriptModel(override);
  ...
  try {
    const parsed = validateLongScriptResult(scriptResultFromUnknown(JSON.parse(content)), theme, duration, style, detailLevel, durationInfo);
    if (!parsed) throw new Error('AI 返回的脚本为空');
    return validateStructuredScript(parsed, contractContext(provider, model));
  } catch {
    // 容错：部分模型会用 ```json 包裹，剥掉再试
    const m = content.match(/\{[\s\S]*\}/);
    if (m) {
      try {
        const parsed = validateLongScriptResult(scriptResultFromUnknown(JSON.parse(m[0])), theme, duration, style, detailLevel, durationInfo);
        if (!parsed) throw new Error('AI 返回的脚本为空');
        return validateStructuredScript(parsed, contractContext(provider, model));
      } catch (error: unknown) {
        if (errorCode(error) === 'SCRIPT_OUTPUT_INVALID') throw error;
      }
    }
 ...
 */
async function expandDialog(dialog: unknown, opts: DialogOptions = {}): Promise<string> {
  if (isDemoMode()) {
 ...
 */
async function optimizeTheme(theme: unknown, opts: ThemeOptions = {}): Promise<string> {
  if (isDemoMode()) {
