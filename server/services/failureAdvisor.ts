...
type FailureStage = 'script' | 'image' | 'voice' | 'compose' | 'asset' | 'storage' | 'queue' | 'unknown'
...
function isFailureStage(value: string): value is FailureStage {
  return value === 'script' || value === 'image' || value === 'voice' || value === 'compose'
    || value === 'asset' || value === 'storage' || value === 'queue' || value === 'unknown';
}

export function stageFromText(text: unknown, stageHint: unknown = ''): FailureStage {
  const s = String(stageHint || '').toLowerCase();
  if (isFailureStage(s)) return s;
...
function stageTitle(stage: FailureStage): string {
  const titles: Record<FailureStage, string> = {
    script: '文案/分镜生成失败',
    image: '图片生成失败',
    voice: '配音生成失败',
    compose: '视频合成失败',
...
function partialResult(projectId: string | number | null | undefined) {
...
function classify(raw: unknown, stage: FailureStage): FailureClassification {
...
export function diagnose(error: unknown, options: DiagnoseOptions = {}) {
  ...
  return {
    ...
    assetHealth: options.assetHealth || errorRecord.assetHealth || null,
