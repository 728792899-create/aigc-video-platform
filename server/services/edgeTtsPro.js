 ...
 */
function buildSsml({ voice, text, rate = 'default', pitch = 'default', volume = 'default', lang = 'zh-CN' }) {
 ...
 */
function synthViaWs(ssml, { outputFormat = 'audio-24khz-48kbitrate-mono-mp3', timeout = 35000 } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const finish = (err) => {
      ...
      err ? reject(err) : resolve({ audioBuffer: Buffer.concat(chunks), words });
    };
    ws.on('open', () => {
    ...
    ws.on('message', (data, isBinary) => {
      if (isBinary) {
        ...
        if (msg.includes('Path:audio.metadata')) {
          try {
            const meta = JSON.parse(msg.split('\r\n').pop());
            (meta.Metadata || []).forEach((el) => {
              if (el.Type === 'WordBoundary' && el.Data && el.Data.text) {
                words.push({
                  part: el.Data.text.Text,
                  start: Math.floor(el.Data.Offset / 10000),
                  end: Math.floor((el.Data.Offset + el.Data.Duration) / 10000),
                });
              }
            });
          } catch {}
    ...
    ws.on('close', (code, reason) => { if (!done) finish(new Error(`edgeTtsPro WebSocket 关闭 code=${code} reason=${(reason || '').toString().slice(0, 80)}`)); });
  });
}

module.exports = { EMOTION_PRESETS, escapeXml, buildSsml, rateStrOf, pitchStrOf, volumeStrOf, synthViaWs };
