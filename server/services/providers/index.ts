...
export type ProviderKind = 'llm' | 't2i' | 't2v' | 'tts'
...
interface BuiltinCredentialProvider {
  builtinDeepSeek(): { apiKey?: string; baseUrl?: string } | null
  builtinZhipu?: () => { apiKey?: string; baseUrl?: string } | null
}
...
let builtinCreds: BuiltinCredentialProvider | null = null;
try { builtinCreds = require('../builtinCreds'); } catch (_) { builtinCreds = null; }

/** 取某 provider 的静态定义 */
export function getProvider(name: string): ProviderDefinition | null {
  return PROVIDERS[name] || null;
}

/** 按能力类型列出 provider（不含密钥） */
export function listByKind(kind: ProviderKind) {
 ...
 * baseUrl 同理回退到注册表默认值。
 */
export function resolveCredentials(name: string): ProviderCredentials | null {
...
export function hasUserCredentials(name: string): boolean {
  const def = PROVIDERS[name];
  if (!def) return false;
  const credKey = def.credentialFrom || name;
